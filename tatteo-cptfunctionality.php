...
function artist_cpt()
{
	...
	$labels = array(
		...
		'add_new_item'          => __('Add Your Artist Profile', 'text_domain'),
		'add_new'               => __('Add Artist', 'text_domain'),
		'new_item'              => __('New Artist', 'text_domain'),
		'edit_item'             => __('Edit Your Artist Profile', 'text_domain'),
	...
	$args = array(
		...
		'labels'                => $labels,
		'supports'              => array('custom-fields', 'comments', 'title'),
...
function events_cpt()
{
	...
	$args = array(
		...
		'labels'                => $labels,
		'supports'              => array('custom-fields', 'page-attributes', 'post-formats', 'comments'),
		'taxonomies'            => array('category', 'post_tag', 'title'),
		'hierarchical'          => true,
		'public'                => true,
		'show_ui'               => true,
		'show_in_menu'          => true,
...
function tatteo_cpt_search($query)
{

	if ($query->is_search) {
		$query->set('post_type', array('studio', 'artist', 'event'));
	}

	return $query;
}
add_filter('pre_get_posts', 'tatteo_cpt_search');
